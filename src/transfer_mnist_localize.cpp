...
void cvLocalizeTwoDigitMNIST(CvMat * training, CvMat * training_twodigit, 
...
int main(int argc, char * argv[])
{
  ...
  CvMat * response_twodigit = cvCreateMat(trainsize,4,CV_32F);
  CvMat *  testing_twodigit = cvCreateMat( testsize,imsize*imsize,CV_32F);
  CvMat * expected_twodigit = cvCreateMat( testsize,4,CV_32F);

  cvLocalizeTwoDigitMNIST(training, training_twodigit, response, response_twodigit);
  cvLocalizeTwoDigitMNIST( testing,  testing_twodigit, expected, expected_twodigit);
                             ...
                             CvMat * response, CvMat * response_twodigit)
{
  ...
  CV_ASSERT(response_twodigit->cols==4);
  ...
  CvRNG rng = cvRNG(-1); int tidx = 0; int vals[4] = {0,0,0,0};
  CvMat vmat = cvMat(1,4,CV_32S,vals);
  ...
  for (int idx=0;idx<training_twodigit->rows;idx++){
    ...
    vals[0] = cvRound(-warp_p->data.fl[2]+28*.5);
    vals[1] = cvRound(-warp_p->data.fl[5]+28*.5);
    tidx = cvFloor(cvRandReal(&rng)*training->rows);
    memcpy(sample->data.ptr,training->data.ptr+training->step*tidx,training->step);
    warp_p->data.fl[0]=warp_p->data.fl[4]=1;
